...
/** Controller class for the UpdateAppointmentView.fxml file. */
public class UpdateAppointmentController implements Initializable {
    ...
    @FXML
    private Label localBusinessHoursLabel;
    @FXML
    ...
    /** Method for populating the Update Appointment input fields with information from
     the Appointment that was selected in the AppointmentTable page. Both startDate and endDate
     datePickers are fired along with the appStartTimeComboBox to ensure proper functionality
     of valid date selections.
     @param appointment The selected Appointment object. */
    public void sendAppointment(Appointment appointment, ActionEvent event){
        appIdTextField.setText(String.valueOf(appointment.getId()));
        appTitleTextField.setText(appointment.getTitle());
        appDescriptionTextField.setText(appointment.getDescription());
        appLocationTextField.setText(appointment.getLocation());
        appTypeTextField.setText(appointment.getType());
        appStartDatePicker.setValue(LocalDate.from(appointment.getStart_date_with_zone()));
        appStartDatePicker.fireEvent(event);
        appStartTimeComboBox.setValue(TimeHandler.hourMinAmPmFormatter.format(appointment.getStart_date_with_zone()));
        appStartTimeComboBox.fireEvent(event);
        appEndDatePicker.setValue(LocalDate.from(appointment.getEnd_date_with_zone()));
        appEndDatePicker.fireEvent(event);
        appEndTimeComboBox.setValue(TimeHandler.hourMinAmPmFormatter.format(appointment.getEnd_date_with_zone()));
        appCustIdComboBox.setValue(appointment.getCustomer_id());
        appUserIdComboBox.setValue(appointment.getUser_id());
        for(Contact contact : appContactComboBox.getItems()){
            if (appointment.getContact_id() == contact.getContact_id()){
                appContactComboBox.setValue(contact);
            }
        }
    }

    /** Method for returning to the AppointmentTable page without updating an Appointment in the database.
    ...
    public void onActionUpdateAppointment(ActionEvent event) throws SQLException, IOException {
        ...
        if (exceptCount == 0) {
            ...
            int currentAppId = Integer.parseInt(appIdTextField.getText());

            for (Appointment appointment : AppointmentTableList.getAllAppointments()) {

                if (appointment.getCustomer_id() == appCustIdComboBox.getValue() && (appointment.getId() != currentAppId) ) {
            ...
            if (exceptCount == 0) {
                ...
                String inputSQLStartDateTime = TimeHandler.toUtcDateTimeString(localZoneStartDateTime);
                String inputSQLEndDateTime = TimeHandler.toUtcDateTimeString(localZoneEndDateTime);
                ...
                AppointmentSQL.update(currentAppId, inputTitle, inputDescription, inputLocation, inputType, inputSQLStartDateTime, inputSQLEndDateTime, inputCustId, inputUserId, inputContactId);
